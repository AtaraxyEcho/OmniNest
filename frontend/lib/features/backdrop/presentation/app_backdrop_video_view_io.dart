...
class _AppBackdropVideoViewState extends ConsumerState<AppBackdropVideoView>
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ref
        .read(appBackdropVideoSessionProvider.notifier)
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final visible = hasController && session.ready && session.renderable;
        final keepMountedOpacity = hasController && (visible || session.ready);
        if (!hasController) {
  ...
  void _syncSession({
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final notifier = ref.read(appBackdropVideoSessionProvider.notifier);
      notifier.setLayoutUsable(layoutUsable);
      notifier.configure(path: source, muted: muted, active: active);
    });
  }
}
